...
function findPage(page, preferredPlatform, preferredLanguage) {
  ...
  return getShortIndex().then((idx) => {
    ...
    if (hasPlatformLang(targets, preferredPlatform, preferredLanguage)) {
      ...
      console.log(
        `Command ${page} does not exist for the host platform. Displaying the page from ${targets[0].os} platform`
      );
  ...
  return getShortIndex().then((idx) => {
    let commands = Object.keys(idx)
      .filter((cmd) => {
        ...
        let platforms = targets.map((t) => {
          return t.os;
        });
        return (
          platforms.indexOf(platform) !== -1 ||
          platforms.indexOf('common') !== -1
        );
      })
      .sort();
    return commands;
